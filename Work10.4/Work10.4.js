...
buttonNext.onclick = (e) => {
    if (currentPage < Math.ceil(locations.length / 10)) {
        currentPage = currentPage + 1;
        pageSpan.innerText = currentPage;
        changeList()
    }
...
const changeList = () => {
    listDiv.innerHTML = ''
    let newArr = locations.slice(((currentPage - 1) * 10), (currentPage * 10));
    for (let i = 0; i < newArr.length; i++) {
        const listItemDiv = document.createElement('div');
        listItemDiv.innerText = `Title: ${newArr[i].title}, Type: ${newArr[i].type}`
